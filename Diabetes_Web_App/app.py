...
FEATURES = [
    ...
    {"name": "HvyAlcoholConsump", "label": "צריכת אלכוהול כבדה", "type": "switch", "category": "אורח חיים",
     "tooltip": "האם את/ה צורך/ת אלכוהול בכמויות גדולות?"},
    {"name": "MentHlth", "label": "ימי בריאות נפש ירודה", "type": "number", "min": 0, "max": 30,
     "category": "חברה וגישה לרפואה", "tooltip": "בכמה ימים בחודש האחרון חשת שהבריאות הנפשית שלך ירודה?"},
    {"name": "PhysHlth", "label": "ימי בריאות פיזית ירודה", "type": "number", "min": 0, "max": 30,
     "category": "חברה וגישה לרפואה", "tooltip": "בכמה ימים בחודש האחרון חשת שהבריאות הפיזית שלך ירודה?"},
    {"name": "AnyHealthcare", "label": "ביטוח בריאות", "type": "switch", "category": "חברה וגישה לרפואה",
     "tooltip": "האם יש לך גישה לשירותי בריאות?"},
    {"name": "NoDocbcCost", "label": "ויתור על רופא עקב עלות", "type": "switch", "category": "חברה וגישה לרפואה",
     "tooltip": "האם נמנעת מביקור אצל רופא עקב עלות?"},
...
dummy_data_dict['GenHlth'] = np.random.randint(1, 6, 100)
dummy_data_dict['MentHlth'] = np.random.randint(0, 31, 100)
dummy_data_dict['PhysHlth'] = np.random.randint(0, 31, 100)
dummy_data_dict['Education'] = np.random.randint(1, 5, 100)
dummy_data_dict['Income'] = np.random.randint(1, 6, 100)
    ...
    if request.method == 'POST':
        ...
        if selected_model == 'xgboost':
            ...
            df_dl['LifeThreateningCondition'] = ((df_dl['HeartDiseaseorAttack'] == 1) | (df_dl['Stroke'] == 1)).astype(
                int)
            df_dl['HealthyLifestyleScore'] = (1 - df_dl['Smoker']) + df_dl['Fruits'] + df_dl['Veggies'] + df_dl[
                'PhysActivity'] + (1 - df_dl['HvyAlcoholConsump'])
